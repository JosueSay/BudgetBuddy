...
def backupManifests(trash_run_dir: Path):
    # copia los manifests antes de cualquier cambio
...
def restoreManifestsFromBackup(trash_run_dir: Path):
    # recupera manifests previos si existen
...
def die(msg, code=1):
    # salida rápida en caso de error
    print(f"error: {msg}")
    sys.exit(code)


def moveToTrash(src_path: Path, trash_run_dir: Path):
    # mueve archivo a papelera manteniendo nombre único
...
def printGroup(group_df: pd.DataFrame, group_key: str, by: str):
    # muestra una vista compacta de un grupo de duplicados
    print("\n" + "=" * 60)
    print(f"grupo: {group_key}  |  criterio: {by}")
    print("-" * 60)
    for idx, row in group_df.reset_index(drop=True).iterrows():
        print(
            f"[{idx}] zip={row['zip_root']}  year={row['year']}  block={row['block']}  size={row['size_bytes']}  path={row['pdf_path']}"
        )
    print("=" * 60)


def refreshManifests(compute_hash: bool):
    # reescanea pdfs para generar manifests consistentes
    ...
    # construye tabla de duplicados según el criterio disponible
    keep_cols = [
        "pdf_filename", "zip_root", "year", "block", "size_bytes",
        "pdf_path", "first_seen_zip", "is_name_duplicate"
    ]
...
def loadDuplicates(by: str, pattern: str | None):
    # carga y filtra duplicados según criterio
    if not DUPS.exists():
...
def interactiveResolve(by: str, groups: dict, apply: bool, compute_hash: bool):
    # modo interactivo para elegir qué pdf conservar
    ...
    while True:
        ...
        for _, r in to_drop.iterrows():
            ...
            log_rows.append({
                ...
                "removed_pdf_original_path": r["pdf_path"],
...
def undoRun(run_dir: Path):
    # revierte la corrida moviendo archivos de vuelta
    ...
    # restaura manifests para volver al estado original
    restoreManifestsFromBackup(run_dir)
    print(f"manifests restaurados desde backup (si estaban disponibles):\n- {MANIFEST}\n- {DUPS}")


def main():
    # cli simple para ejecutar el flujo
    ap = argparse.ArgumentParser()
    ap.add_argument("--by", choices=["name", "hash"], default="name", help="criterio de duplicado")
    ap.add_argument("--filter", type=str, default=None, help="regex para filtrar grupos")
    ap.add_argument("--apply", action="store_true", help="aplica cambios")
    ap.add_argument("--undo", type=str, help="ruta a corrida en .trash para revertir")
    args = ap.parse_args()
